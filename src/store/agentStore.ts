...
export const useAgentStore = create<AgentState>()(
  ...
  handleLogin: async (platform: string) => {
      ...
      try {
          ...
          if (platform === 'facebook') {
              ...
              token = await WhatsAppService.login();
              if (token) accounts = await WhatsAppService.getPhoneAccounts(token);
          }
  ...
  handleAccountSelection: async (platform: string, account: any) => {
      ...
      try {
          ...
          else if (platform === 'instagram') await InstagramService.saveConfig(account.id, account.access_token || accessToken, account.username, accessToken, account.page_id);
          ...
          else if (platform === 'whatsapp') await WhatsAppService.saveConfig(account.id, accessToken, account.name);
