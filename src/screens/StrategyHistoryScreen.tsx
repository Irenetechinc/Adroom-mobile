...
function PerformancePanel({ strategyId }: { strategyId: string }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`strategy-performance-${strategyId}`)
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'agent_performance', filter: `strategy_id=eq.${strategyId}`,
      }, () => { loadPerf(); })
      .subscribe();
...
function DemographicPanel({ strategyId }: { strategyId: string }) {
  ...
  useEffect(() => {
    loadIntel();
    const channel = supabase
      .channel(`strategy-demographics-${strategyId}`)
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'platform_intelligence', filter: `strategy_id=eq.${strategyId}`,
      }, () => { loadIntel(true); })
      .subscribe();
    return () => {
      clearPolling();
...
export default function StrategyHistoryScreen() {
  ...
  useEffect(() => {
    ...
    const connectRealtime = async () => {
      ...
      channel = supabase
        .channel(`strategy-history-${user.id}`)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'strategies', filter: `user_id=eq.${user.id}` }, () => { fetchHistory(); })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'agent_leads', filter: `user_id=eq.${user.id}` }, () => { fetchHistory(); })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'agent_tasks', filter: `user_id=eq.${user.id}` }, () => { fetchHistory(); })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'strategy_conversation_runs', filter: `user_id=eq.${user.id}` }, () => { fetchHistory(); })
        .on('postgres_changes', { event: '*', schema: 'public', table: 'strategy_conversation_signals', filter: `user_id=eq.${user.id}` }, () => { fetchHistory(); })
        .subscribe();
      refreshTimer = setInterval(() => { fetchHistory(); }, 60_000);
    };
    connectRealtime();
    return () => {
      mounted = false;
