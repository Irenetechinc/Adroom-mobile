...
export default function DashboardScreen() {
  ...
  useEffect(() => {
    ...
    const intelligenceChannel = supabase
      ...
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'strategy_conversation_runs',
        filter: `user_id=eq.${session.user.id}`,
      }, () => { fetchData(); })
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'strategy_conversation_signals',
        filter: `user_id=eq.${session.user.id}`,
      }, () => { fetchData(); })
      .on('postgres_changes', {
              ...
              >
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                    ...
                    <Text style={{ color: '#64748B', fontSize: 11, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.8 }}>Adirum Energy</Text>
                  </View>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
