...
export class PlatformIntelligenceEngine {
  ...
  private async dispatchIntelligence(items: any[]) {
    ...
    for (const item of items) {
      ...
      // 2. If Urgent (Priority 1), trigger notifications via Communication Engine
      if (item.impact_score >= 8 && inserted) {
          ...
          try {
             ...
             await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/communication-engine`, {
                 ...
                 body: JSON.stringify({
                     action: 'generate_alert_message',
                     alertId: inserted.id
                 })
             });
          } catch (commError) {
             console.error("Failed to trigger communication engine:", commError);
          }
      }
    }
  }
