...
## Recent Updates (v2.2.10 — 2026-04-29)
- **In-app "What's New" changelog**: After every app update the user sees a one-time modal listing every release note between the version they last acknowledged and the version they're now running. Notes are stored in the new `app_releases` Supabase table (markdown bullets, per platform) and served via the public `GET /api/app/version` endpoint. The last-seen version is persisted in AsyncStorage under `adroom:lastSeenChangelogVersion`. Fresh installs are silently marked seen so first-time users aren't greeted with a "What's New" modal. New file: `src/components/WhatsNewModal.tsx`.
- **Force-update gate**: On every cold launch (and again on foreground after 30+ min idle) the app calls `GET /api/app/version?platform=android&current=<x.y.z>`. If the device's version is below the highest `is_min_supported = TRUE` row, a non-dismissable blocking modal is rendered over the entire app with a single "Update now" button that opens the Play Store / App Store. If a newer version exists but the install is still supported, a soft "Update available" prompt is shown instead (dismissable, not re-shown again in the same session). New files: `src/components/ForceUpdateModal.tsx`, `src/services/appVersionService.ts`. Wiring lives in `App.tsx`.
- **New endpoint `GET /api/app/version`** (public, unauthenticated — must work before sign-in): Returns `{ currentVersion, latestVersion, minSupportedVersion, storeUrl, updateAvailable, forceUpdate, changelog: [{version, releasedAt, notes}] }`. Fails open: if Supabase is unreachable or the table is missing, the endpoint returns empty/null fields so the app is never bricked.
- **New Supabase migration `20260429000000_app_releases.sql`**: Creates `public.app_releases (id, platform, version, is_min_supported, is_published, force_update, store_url, changelog_md, released_at, created_at)` with public-read RLS, unique `(platform, version)` constraint, and partial indexes for fast version lookups. Seeded with the v2.2.7 row containing the real release notes (push fix + diagnostic + landing forms + responsive landing pages + What's New + force-update). **The team must apply this migration to Supabase** (via Supabase CLI `supabase db push` or pasting the SQL into the Supabase SQL editor) for the changelog and force-update features to start returning real data — until then the endpoint correctly returns empty results and both modals stay dormant.
- **Shipping a new release going forward**: bump `expo.version` in `app.json`, then `INSERT INTO app_releases (platform, version, is_min_supported, store_url, changelog_md) VALUES (...)`. Set `is_min_supported = TRUE` only on the row whose version should become the new minimum. Old clients are auto-prompted on next launch.
